...
class InworldService {
                    ...
                    try {
                        ...
                        if (data.result && data.result.audioContent) {
                            ...
                            if (!firstAudioChunkReceived) {
                                ...
                                if (this.vadService) {
                                    this.audioManager.saveChunkToDisk(sessionId, 'inworld', audioData, totalAudioChunks, 'first_chunk');
                                }
        ...
        response.data.on('end', async () => {
            ...
            if (audioChunks.length > 0) {
                ...
                hasAudio = true;
                    ...
                    completeAudioPath = this.audioManager.saveCompleteAudio(sessionId, 'inworld', audioBuffer);
    ...
    /**
     * Simulate Inworld TTS when no API key is provided
     * @param {string} text - Text to convert to speech
     * @param {Function} sendUpdate - Function to send progress updates
